...
The system SHALL reject configured file audit paths that are empty, whitespace-only, untrimmed, contain ASCII control characters, or exceed 1024 UTF-8 bytes before writing audit records or falling back to non-file audit behavior. Audit path validation failures MUST NOT include the raw configured path value in thrown errors, usage text, logs, or audit output.
