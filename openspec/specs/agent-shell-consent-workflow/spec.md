...
#### Scenario: Malformed token-env option is rejected
- **WHEN** the agent shell is started with a malformed `--token-env` name, an unset token environment variable, an unsafe resolved token value, or both `--token` and `--token-env`
- **THEN** it exits through bounded usage handling before connecting to the relay or sending any protocol message
- **AND** diagnostics MUST NOT expose token values, token environment values, credentials, pairing codes, or full secrets
