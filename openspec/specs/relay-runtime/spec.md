...
The relay runtime SHALL include bounded device identity metadata in accepted `relay.peer.join.accepted` audit detail when a peer joins with schema-valid `deviceIdentity`. The accepted join audit metadata MUST NOT include raw display names, raw pairing codes, tokens, credentials, protocol payloads, keystrokes, screenshots, screen contents, or full secrets. When a schema-valid `deviceIdentity.deviceId` contains the submitted pairing code, the relay MUST NOT include the raw `deviceId` and MUST include bounded redaction metadata instead. Device identity audit metadata MUST remain non-authorizing and MUST NOT grant screen, input, clipboard, file, diagnostics, reconnect, hidden-session, or consent-bypass permissions.

#### Scenario: Accepted host join includes bounded device identity
- **WHEN** a host joins the relay with schema-valid device identity metadata whose `deviceId` does not contain the submitted pairing code
...
- **WHEN** a viewer joins the relay with schema-valid device identity metadata whose `deviceId` does not contain the submitted pairing code after consuming a host pairing ticket
- **THEN** the accepted join audit detail includes `deviceIdentity.deviceId`, `deviceIdentity.platform`, `deviceIdentity.trustLevel`, and `deviceIdentity.createdAt`
- **AND** the accepted join audit detail MUST NOT include the viewer display name or raw pairing code

#### Scenario: Accepted join redacts device id containing pairing code
- **WHEN** an accepted join's schema-valid device identity has a `deviceId` containing the submitted pairing code
- **THEN** the accepted join audit detail MUST NOT include the raw `deviceId`
- **AND** the accepted join audit detail includes bounded redaction metadata for that `deviceId`
- **AND** the raw submitted pairing code MUST NOT appear anywhere in the audit record
