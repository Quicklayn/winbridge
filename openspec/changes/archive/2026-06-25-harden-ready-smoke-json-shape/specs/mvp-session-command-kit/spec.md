...
in the aggregate ready output. Top-level smoke JSON consumed by the ready
helper MUST be rejected unless it contains only fixed bounded fields needed for
aggregate readiness: `ok`, `checks`, optional bounded `reason` on failure, and
the default cleanup marker `artifacts` with value `cleaned` on success. Smoke
...
#### Scenario: Ready helper rejects smoke JSON with unexpected top-level fields

- **WHEN** `npm run mvp:ready -- --include-smoke` receives smoke JSON where the
  top-level object includes an unexpected field such as an artifact directory,
  URL, token, command, raw output, or private diagnostic
...
#### Scenario: Ready helper accepts default cleaned smoke artifacts marker

- **WHEN** `npm run mvp:ready -- --include-smoke` receives successful smoke JSON
  with `artifacts` set to `cleaned`
- **THEN** the ready helper may accept the smoke subchecks for aggregate
  readiness
- **AND** ready output MUST NOT echo artifact paths or retained artifact
  metadata
