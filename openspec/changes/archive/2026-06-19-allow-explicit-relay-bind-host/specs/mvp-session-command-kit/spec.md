...
starting relay. The printed relay command SHALL use a root helper that builds
the workspace packages required by the development relay before starting relay.
The printed host command SHALL require an explicit host action to run, keep the
session visible, prompt the host interactively before approval, enable
...
#### Scenario: Non-loopback relay URL prints explicit bind host
- **WHEN** the command kit prints commands for a validated non-loopback relay URL
- **THEN** the relay command explicitly sets `WINBRIDGE_RELAY_BIND_HOST` to `0.0.0.0`
- **AND** the command kit itself MUST NOT probe network interfaces, open firewall ports, start relay, host, viewer, browser, capture, input, socket, HTTP, service, startup persistence, privilege, unattended, or Windows prompt actions
