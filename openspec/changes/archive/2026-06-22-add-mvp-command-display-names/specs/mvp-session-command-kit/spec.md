...
bounded viewer display-name metadata for the host consent prompt, a viewer
latest-frame output path, a loopback local viewer control surface port, a
browser open command for that loopback surface, and host-side pause, resume,
revoke, terminate, and disconnect controls. When invoked with `--relay-host`,
the command kit MUST accept only a bounded host name or IPv4 literal without a
...
#### Scenario: Command plan includes host-facing viewer display name

- **WHEN** a developer runs the command kit with default display-name options
- **THEN** the printed host and viewer commands include explicit bounded
  `--name` values
- **AND** the viewer name is available to the existing host consent prompt as
  non-authorizing display metadata

#### Scenario: Custom display names are rendered

- **WHEN** a developer runs the command kit with `--host-name Assisted PC` and
  `--viewer-name Support Viewer`
- **THEN** the printed host command includes `--name 'Assisted PC'`
- **AND** the printed viewer command includes `--name 'Support Viewer'`
- **AND** the helper does not treat either name as authentication or
  authorization
...
combinations, unsafe host or viewer display names, and invalid JSON/preflight
flags without echoing raw unsafe input in diagnostics. The `--relay-host`
shortcut MUST be rejected when it is malformed, loopback, unspecified,
secret-bearing, or combined with `--relay`. The
...
#### Scenario: Unsafe display name is rejected

- **WHEN** a developer supplies a blank, untrimmed, oversized,
  control-character, format-control, or secret-bearing `--host-name` or
  `--viewer-name` value
