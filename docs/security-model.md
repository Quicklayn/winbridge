...
- Viewer control prompt mode is an opt-in development CLI surface for exact commands `help`, `status`, and `disconnect`. It is viewer-only, mutually exclusive with one-shot viewer status and disconnect timers, does not echo raw command lines, prints only a static command list for `help`, reads only the bounded local viewer status snapshot for `status`, including optional local inactive cause metadata after explicit viewer leave or local viewer socket close, and invokes only the managed viewer-only `leave()` control for `disconnect`. After successful `disconnect`, the prompt stops locally and no longer accepts further command input for the left viewer session; failed disconnect attempts keep the prompt available with sanitized error output. It must not invoke host controls, send forged `peer-disconnected`, send lifecycle/signal/control/workflow audit messages, grant permissions, start signaling, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, or private-reason data.
...
The shell never captures the screen, renders a remote desktop UI, injects OS input, syncs clipboard, transfers files, installs services, or enables unattended access. Its development remote interaction methods send only caller-supplied frame bytes or input intents after authorization, routing, permission, and audit gates.
...
Configured relay or agent audit file paths must be non-blank, already trimmed, 1024 UTF-8 bytes or less, contain no ASCII control characters, contain no Unicode bidi/zero-width formatting controls, contain no Windows reserved device path segments such as `NUL`, `CON`, `COM1`, or `LPT1`, contain no Windows alternate data stream path segments such as `logs\audit.jsonl:hidden`, and not use Windows device namespace prefixes such as `\\.\` or `\\?\`. Omitted audit paths keep the documented development fallback behavior, but empty, whitespace-only, untrimmed, control-character, bidi/zero-width-control, oversized, Windows-device, Windows-stream, or Windows-device-namespace configured paths fail before relay peer acceptance or agent runtime startup without exposing the raw configured path value.
