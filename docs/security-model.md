...
Configured relay or agent audit file paths must be non-blank, already trimmed, 1024 UTF-8 bytes or less, contain no ASCII control characters, contain no Unicode bidi/zero-width formatting controls, and contain no Windows reserved device path segments such as `NUL`, `CON`, `COM1`, or `LPT1`. Omitted audit paths keep the documented development fallback behavior, but empty, whitespace-only, untrimmed, control-character, bidi/zero-width-control, oversized, or Windows-device configured paths fail before relay peer acceptance or agent runtime startup without exposing the raw configured path value.
