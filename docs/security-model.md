...
- Public runtime sends for workflow-authority messages (`session-authorization-decision`, `session-authorization-state`, `permission-revoked`, `session-control`, and `audit-event`) are rejected before socket write and local `sent` event emission; blocked-send errors, events, and logs do not include raw protocol payloads, private reasons, or audit details.
