...
- Host control prompt mode is an opt-in development CLI surface for exact commands `help`, `status`, `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`. It is host-only, mutually exclusive with interactive consent prompt mode, does not echo raw command lines, prints only a static command list for `help`, uses a read-only bounded runtime snapshot for `status`, and invokes only managed runtime direct controls for lifecycle commands, so the existing visible authorization, expiration, terminal-state, audit, indicator, and disconnect gates remain authoritative.
- Viewer status snapshots are read-only, viewer-only, and bounded to local lifecycle metadata such as state, host visibility, permission count, and optional authorization id/status. After trusted host disconnect, viewer status reports inactive local state, host visibility false, and zero action-capable permissions without rewriting the last authorization status. After managed local viewer leave, viewer status reports inactive local state, host visibility false, and zero action-capable permissions while omitting authorization id/status metadata from the left connection scope. The development CLI can print status once with `--viewer-status-after-ms <delay>`, which is viewer-only, accepts exact bounded millisecond delays, and does not require requested permissions. Reading or printing viewer status does not send protocol messages, emit workflow audit events, grant permissions, start signaling, invoke host controls, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, or private-reason data.
