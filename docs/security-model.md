...
- The managed runtime rejects malformed direct options for relay URL, relay URLs with embedded credentials or canonical/case-variant `token` query values, identifiers, blank, untrimmed, control-character, bidi/zero-width-control, oversized, or secret-bearing display name, blank, untrimmed, control-character, bidi/zero-width-control, non-string, or oversized token, requested permissions, host grant scope, revoke permission, visible-session flag, host decision, viewer runtime host workflow state, invalid host decision providers, host decision providers on viewer runtimes, host decision providers combined with static approval/denial, providerless, zero, or unsafe host consent timeout, zero or unsafe authorization TTL, workflow timers, viewer signal probe configuration, host signal probe acknowledgement configuration, and blank, untrimmed, control-character, bidi/zero-width-control, oversized, or secret-bearing decision/lifecycle reasons before opening a relay connection or sending any authorization decision. It also rejects duplicate active `start()` calls before opening another WebSocket, sending protocol messages, emitting local protocol events, granting permissions, activating host visibility, or reconnecting peers, while preserving explicit restart after a fully closed or stopped connection. Relay shared tokens must use the dedicated `--token`/runtime token path and are bounded before connection setup.
