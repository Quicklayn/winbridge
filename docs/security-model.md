...
- Viewer status snapshots are read-only, viewer-only, and bounded to local lifecycle metadata such as state, host visibility, permission count, and optional authorization id/status. After trusted host disconnect, viewer status reports inactive local state, host visibility false, and zero action-capable permissions without rewriting the last authorization status. The development CLI can print status once with `--viewer-status-after-ms <delay>`, which is viewer-only, accepts exact bounded millisecond delays, and does not require requested permissions. Reading or printing viewer status does not send protocol messages, emit workflow audit events, grant permissions, start signaling, invoke host controls, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, or private-reason data.
- Viewer local disconnect is an opt-in development CLI leave path through `--viewer-disconnect-after-ms <delay>`. It is viewer-only, accepts exact bounded millisecond delays, does not require requested permissions or active authorization, and stops only the local viewer runtime. It must not send forged `peer-disconnected`, lifecycle, signal, control, or workflow audit messages; the relay observes the socket close and notifies the remaining host.
- Viewer control prompt mode is an opt-in development CLI surface for exact commands `status` and `disconnect`. It is viewer-only, mutually exclusive with one-shot viewer status and disconnect timers, does not echo raw command lines, reads only the bounded local viewer status snapshot for `status`, and stops only the local viewer runtime for `disconnect`. It must not invoke host controls, send forged `peer-disconnected`, send lifecycle/signal/control/workflow audit messages, grant permissions, start signaling, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, or private-reason data.
