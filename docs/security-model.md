...
- Host control prompt mode is an opt-in development CLI surface for exact commands `status`, `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`. It is host-only, mutually exclusive with interactive consent prompt mode, does not echo raw command lines, uses a read-only bounded runtime snapshot for `status`, and invokes only managed runtime direct controls for lifecycle commands, so the existing visible authorization, expiration, terminal-state, audit, indicator, and disconnect gates remain authoritative.
