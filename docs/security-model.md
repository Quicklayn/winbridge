...
Protocol-facing machine identifiers such as session ids, peer ids, message ids, authorization ids, pairing ids, device ids, and audit event ids are bounded and restricted to a safe printable profile before relay registration, forwarding, authorization, pairing, or audit-related protocol use. Protocol envelope message ids also reject secret-bearing marker metadata such as tokens, credentials, pairing codes, authorization headers, cookies, private keys, SSH keys, and authorization metadata before parsing, encoding, relay forwarding, accepted-forward audit, or runtime use. Device identity ids, relay peer-derived fallback device ids, pairing ticket ids, pairing ticket session ids, pairing ticket host device ids, and paired-device relationship ids also reject secret-bearing marker metadata before identity creation, pairing ticket creation or consumption, paired-device recording, relay registration, accepted join audit, or denied join audit. Agent-shell CLI and managed runtime session ids, peer ids, and device ids also reject secret-bearing marker metadata before relay startup.
