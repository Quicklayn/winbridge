...
- Host control prompt mode is an opt-in development CLI surface for exact commands `help`, `status`, `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`. It is host-only, mutually exclusive with interactive consent prompt mode and one-shot host status mode, does not echo raw command lines, prints only a static command list for `help`, uses a read-only bounded runtime snapshot for `status`, and invokes only managed runtime direct controls for lifecycle commands, so the existing visible authorization, expiration, terminal-state, audit, indicator, and disconnect gates remain authoritative. The development CLI can also print the same bounded local host status snapshot once with `--host-status-after-ms <delay>`, which is host-only, accepts exact bounded millisecond delays, does not require requested permissions, and may run inside an ordinary host runtime whose startup and explicit workflow options retain their existing protocol behavior. After successful `terminate` or `disconnect`, the prompt stops locally and no longer accepts further command input for the terminated or closed local session; failed attempts keep the prompt available with sanitized error output. Host status reflects inactive local indicator state after deactivation and may include only a bounded local `inactiveCause`; the scheduled status read itself must not reconnect peers, grant permissions, start signaling, invoke controls, emit workflow audit events, send protocol messages, or expose peer ids, display names, raw close reason text, private reasons, tokens, pairing codes, signal payloads, raw protocol data, screen contents, input contents, clipboard contents, file-transfer contents, or diagnostics dumps.
- Viewer status snapshots are read-only, viewer-only, and bounded to local lifecycle metadata such as state, host visibility, permission count, optional authorization id/status, optional `signalProbeAckReceived=true` after a trusted current-authorization host signal acknowledgement, optional relay-defined remote disconnect reason code after trusted host disconnect, and optional local inactive cause after explicit viewer leave or local viewer socket close. Signal acknowledgement status is omitted after authorization loss or local/remote disconnect and does not expose raw signal payload markers, kind metadata, or contents. After trusted host disconnect, viewer status reports inactive local state, host visibility false, zero action-capable permissions, and the bounded disconnect reason code without rewriting the last authorization status. After managed local viewer leave, viewer status reports inactive local state, host visibility false, zero action-capable permissions, and a bounded `localInactiveCause` while omitting authorization id/status metadata from the left connection scope. After local viewer socket close without trusted host disconnect, viewer status reports inactive local state, host visibility false, zero action-capable permissions, and `localInactiveCause=socket-closed` while omitting stale authorization id/status metadata from the closed connection scope. The development CLI can print status once with `--viewer-status-after-ms <delay>`, which is viewer-only, accepts exact bounded millisecond delays, and does not require requested permissions. Reading or printing viewer status does not send protocol messages, emit workflow audit events, grant permissions, start signaling, reconnect peers, invoke host controls, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, signal-kind, display-name, raw close reason text, or private-reason data.
