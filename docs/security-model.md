...
- Host control prompt mode is an opt-in development CLI surface for exact commands `help`, `status`, `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`. It is host-only, mutually exclusive with interactive consent prompt mode, does not echo raw command lines, prints only a static command list for `help`, uses a read-only bounded runtime snapshot for `status`, and invokes only managed runtime direct controls for lifecycle commands, so the existing visible authorization, expiration, terminal-state, audit, indicator, and disconnect gates remain authoritative. After successful `disconnect`, the prompt stops locally and no longer accepts further command input for the closed local session; failed disconnect attempts keep the prompt available with sanitized error output. Host status reflects inactive local indicator state after deactivation and may include only a bounded local `inactiveCause`; it must not reconnect peers, grant permissions, start signaling, invoke controls, or expose peer ids, display names, raw close reason text, private reasons, tokens, pairing codes, signal payloads, or raw protocol data.
