...
- Host local disconnect simulation requires explicit visible approval plus `--disconnect-after-ms`. Managed host runtimes can also invoke direct local disconnect control after active or paused visible authorization. Both paths close the host relay connection and rely on the relay to emit `peer-disconnected`; the host shell must not send forged disconnect notices.
