...
- CLI parsing rejects unknown, duplicate, missing-value, malformed relay URL, malformed protocol identifier, malformed display name, malformed permission, malformed pairing, malformed lifecycle reason, and non-`true`/`false` `--visible-session` values before starting the runtime.
