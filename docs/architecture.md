...
- Interactive host control prompt mode is an opt-in development CLI surface that accepts exact host commands `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`; it is host-only, mutually exclusive with interactive host consent prompt mode, never echoes raw command lines, and calls only the managed runtime direct controls.
