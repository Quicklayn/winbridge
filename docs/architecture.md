...
- CLI argument parsing rejects unknown, duplicate, missing-value, malformed relay URL, relay URLs with embedded credentials or canonical/case-variant `token` query values, malformed protocol identifier, malformed permission, malformed pairing, blank, untrimmed, control-character, or oversized token, zero or unsafe `--authorization-ttl-ms`, blank or untrimmed audit log path, blank, untrimmed, or oversized lifecycle reason, non-`true`/`false` visible-session or host-consent-prompt values, viewer prompt mode, and prompt mode combined with static approval/denial before runtime start.
