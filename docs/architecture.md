...
The shell can exercise non-native development `screen-frame` and `input-event` flows through explicit runtime methods after active, visible, unexpired authorization. The host frame method publishes caller-supplied development frame bytes only; the viewer input method publishes caller-supplied pointer or keyboard intent only. Accepted local sends persist metadata-only audit before socket write, and audit failure blocks the send. Runtime sent/received events redact frame data and input details. The shell still does not capture the screen, render a viewer UI, inject OS input, or bind to native Windows adapters; native behavior requires later OpenSpec changes for host UI, viewer UI, Windows capture, and Windows input adapters.
