...
- Interactive host consent prompts accept only exact `approve` or `deny` responses, are mutually exclusive with static approval/denial, and fail closed on invalid input, cancellation, or prompt failure.
...
- CLI argument parsing rejects unknown, duplicate, missing-value, malformed relay URL, relay URLs with embedded credentials or canonical/case-variant `token` query values, malformed protocol identifier, blank, untrimmed, control-character, bidi/zero-width-control, or oversized display name, malformed permission, malformed pairing, blank, untrimmed, control-character, or oversized token, zero or unsafe `--authorization-ttl-ms`, blank, untrimmed, control-character, or oversized audit log path, blank, untrimmed, or oversized lifecycle reason, non-`true`/`false` visible-session or host-consent-prompt values, viewer prompt mode, and prompt mode combined with static approval/denial before runtime start.
- The managed runtime also rejects malformed direct options before relay startup, including non-WebSocket relay URLs, relay URLs with embedded credentials or canonical/case-variant `token` query values, malformed identifiers, blank, untrimmed, control-character, bidi/zero-width-control, or oversized display names, blank, untrimmed, control-character, non-string, or oversized tokens, duplicate or invalid permissions, non-boolean visible-session flags, invalid host decision providers, host decision providers on viewer runtimes, host decision providers combined with static approval/denial, zero or unsafe authorization TTL, unsafe workflow timers, and blank, untrimmed, or oversized decision/lifecycle reasons. Relay shared tokens use the dedicated `--token`/runtime token path and are bounded before connection setup.
