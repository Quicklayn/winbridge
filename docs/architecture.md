...
- The MVP command kit is a non-executing operator helper. It rejects non-loopback `--relay-host` and `--relay` session plans without bounded `--token-env <NAME>` before rendering relay, host, viewer, browser, JSON, or role-filter runtime commands; loopback and preflight-only plans stay available without token-env. The root readiness helper validates its representative LAN command plan and LAN role-filter checks through the shared-token env path without reading token values or echoing command output.
...
- Viewer local control surface reads only the explicit configured latest-frame output file, returns no-store image responses on loopback, rejects missing frames and mismatched Host headers with bounded metadata, and routes accepted input through the same viewer input send gates as the terminal prompt.
- MVP smoke readiness exercises the live loopback viewer surface guard path, including a fixed mismatched Host-header request that must return bounded rejection metadata before the surface is considered ready.
