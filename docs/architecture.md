...
- Interactive host control prompt mode is an opt-in development CLI surface that accepts exact host commands `help`, `status`, `pause`, `resume`, `revoke <permission>`, `terminate`, and `disconnect`; it is host-only, mutually exclusive with interactive host consent prompt mode, never echoes raw command lines, prints only a static command list for `help`, calls only the managed runtime direct controls for lifecycle commands, and uses a read-only runtime snapshot for `status`.
