...
const sensitiveReasonMarkerPattern =
  /\b(?:token|credential|password|pass[\s_-]*phrase|secret|pairing[\s_-]*code|api[\s_-]*key|access[\s_-]*key|authorization|proxy[\s_-]*authorization|authorization[\s_-]*header|auth[\s_-]*header|set[\s_-]*cookie|session[\s_-]*cookie|cookie|private[\s_-]*key|ssh[\s_-]*key|keystroke|raw[\s_-]*key[\s_-]*log|key[\s_-]*log(?:ger)?(?:[\s_-]*(?:content|data|output|trace))?|screenshot|screen[\s_-]*data|screen[\s_-]*content)\b\s*(?::|=|\s+)\s*\S+/i;
